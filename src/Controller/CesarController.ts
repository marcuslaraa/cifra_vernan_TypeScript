...
class CesarController implements ICesarController {
  ...
  bruteForce = async (request: Request, response: Response, next: NextFunction) => {
    ...
    const result = await this.cesarService.bruteForce(textoCifrado);
    console.log(result);
    response.json({ textoCifrado: result.textoClaro });
  };
